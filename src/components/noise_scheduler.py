...
class NoiseScheduler(nn.Module):
    """
    NoiseSchedule is a module that computes the alpha_bar values
    based on a time tensor t. It uses a parameterized gamma function to adjust
    the alpha_bar values, which are used in diffusion models to control the noise
    schedule during training and inference.
    ...
    def __init__(self, hidden_dim: int = 64):
        """
        Initializes the NoiseSchedule with a hidden dimension for the gamma function.
